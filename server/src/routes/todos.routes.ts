...
router.get("/:email", async (req: Request<{ email: string }, {}, { completed: boolean }>, res: Response<Todo[]>) => {
...
router.get("/:email/all", async (req: Request<{ email: string }>, res: Response<{ [key: number]: { todo: Todo; todoLogs: TodoLog[] } }>) => {
...
router.get("/:email/folder/:folder", async (req: Request<{ email: string; folder: string }, {}, { completed: boolean }>, res: Response<Todo[]>) => {
...
// completed 값에 따라 완료여부를 필터링한다.
router.get("/:email/date/:date", async (req: Request<{ email: string; date: string }, {}, { completed: boolean }>, res: Response<Todo[]>) => {
...
router.post("/:email", async (req: Request<{ email: string }, {}, { folder: string; content: string; dates: string[]; days: boolean[] }>, res: Response) => {
  // email은 로그인된 사용자의 이메일을 가져오므로 항상 있다고 가정한다.
  const writer = req.params.email;
  ...
  if (!folder || !content || !dates || (dates && !dates.length) || !days || (days && !days.length)) {
    return res.status(400).send("folder, content, dates, days 중 하나의 값이 없습니다.");
  }
...
router.patch("/check", async (req: Request<{}, {}, { todoId: number; date: string; completed: boolean }>, res: Response) => {
...
router.delete("/", async (req: Request<{}, {}, { id: number }>, res: Response) => {
  const id = req.body.id;
